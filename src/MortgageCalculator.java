...
public class MortgageCalculator {
    ...
    public static BigDecimal calculateMortgage(int principal, float monthlyInterest, int numPayments) {
        if (monthlyInterest == 0) // Edge case: zero interest rate
            return BigDecimal.valueOf(principal)
                    .divide(BigDecimal.valueOf(numPayments), 2, RoundingMode.HALF_UP);
